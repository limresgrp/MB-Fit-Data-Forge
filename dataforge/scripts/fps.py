...
def compute_all_distances(coords, names, max_processes=1):
...
def furthest_point_sampling(num_samples, coords, names, chunk_max_dim=10000, max_processes=1, n_clusters=10):
    def fps(M, N):
    ...
    while len(coords) > chunk_max_dim:
        ...
        for i, chunk in enumerate(chunks):
            ...
            distance_matrix = compute_all_distances(chunk, names, max_processes=max_processes)
    ...
    distance_matrix = compute_all_distances(coords, names, max_processes=max_processes)
...
def process_h5_file(h5_filename, n_samples, chunk_max_dim, max_processes):
    coords, atom_types, fullnames, info_dict, extra_data = read_h5_file(h5_filename)
    names = extra_data['symmetry_names_sorted']

    sampled_indices = furthest_point_sampling(n_samples, coords, names, chunk_max_dim=chunk_max_dim, max_processes=max_processes)
...
def main():
    ...
    for h5_filepath in h5_filepaths:
        ...
        process_h5_file(h5_filepath, n_samples, chunk_max_dim, max_processes)
