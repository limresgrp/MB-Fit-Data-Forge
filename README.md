...
XYZ sampling and initial capping are separate menu operations. Sampling any selected higher-order n-mer automatically writes its complete connected lower-order closure using the same sampled trajectory frames, even when the lower orders were not entered separately. For example, 1,000 frames of a linear trimer produce two 1,000-frame dimer components and three 1,000-frame monomer components. Components shared by multiple parents are stored once per topological occurrence: repeated trajectory-frame IDs within the sampling operation are deduplicated, while previously unseen parent frames are combined. Rerunning operation 3 replaces the previous sample set so the requested target remains a final target rather than an additional count. For multi-gigabyte trajectories, n-mer construction automatically switches to one build process to avoid multiplying large coordinate arrays across workers. The QChem and capping stages can still use parallel workers.
...
By default, `dataforge-build-nmers`/the workflow uses automatic monomer discovery. Each monomer starts as a heavy atom plus its bonded hydrogens; heavy atoms joined by an inferred or topology-provided double/triple bond are merged into one monomer. Operation 2 asks for a readable name for every `AUTO-*` type; pressing Enter retains the automatic name. Aliases are saved in `metadata/monomer_aliases.json`, reused on later runs, and applied before n-mer paths and metadata are generated.

Operation 2 then lists the inferred monomer types, their occurrence counts, and their suggested integer charges. The user may select two or more type numbers to merge when they form a connected component—for example, combining the inferred phosphorus/oxygen pieces into one phosphate monomer—and supplies a name for the larger monomer. Every occurrence of a selected type belonging to the matching connected component is included, so selecting `O-CP` and `PO2-` includes both phosphate ester oxygens when both are bonded to that phosphorus group. Definitions are validated before being saved in `metadata/monomer_merges.json` and reapplied during subsequent discovery and XYZ builds. An invalid merge produces a readable error and does not modify the saved definitions.

Charge suggestions use integer bond-order sums from the complete topology, not only the atoms retained in the fragment. This recognizes common cases such as terminal singly bonded oxygen (`-1`), four-coordinate nitrogen (`+1`), neutral two-coordinate ester oxygen, and phosphate charge from its protonated/substituted oxygen count. Suggestions remain interactive: they are displayed for review, may be edited, and confirmed values are saved in `metadata/monomer_charges.json`. That file becomes the default charge source during both QChem input-preparation operations. Detailed atom-level charge evidence, special rules, uncertain rules, merge matches, and the connected n-mer catalog are saved in `data/monomer_discovery.json`.

Use `--monomer-mode legacy` only when reproducing a pre-existing configured composite-monomer definition, and provide `--bond-order-mode topology` or `geometry` when you want to force a particular bond-order source.
