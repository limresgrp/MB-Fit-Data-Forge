...
Operation 2 then lists the inferred monomer types and their suggested integer charges. The user may select two or more type numbers to merge when they form a connected component—for example, combining the inferred phosphorus/oxygen pieces into one phosphate monomer—and supplies a name for the larger monomer. Definitions are saved in `metadata/monomer_merges.json` and reapplied during subsequent discovery and XYZ builds. A merge is rejected if the selected types do not occur together as a connected component.

Charge suggestions use integer bond-order sums from the complete topology, not only the atoms retained in the fragment. This recognizes common cases such as terminal singly bonded oxygen (`-1`), four-coordinate nitrogen (`+1`), and neutral two-coordinate ester oxygen. Suggestions remain interactive: they are displayed for review, may be edited, and confirmed values are saved in `metadata/monomer_charges.json`. That file becomes the default charge source during both QChem input-preparation operations. Detailed atom-level charge evidence, uncertain rules, merge matches, and the connected n-mer catalog are saved in `data/monomer_discovery.json`.
